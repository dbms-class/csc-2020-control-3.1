...
class App(object):
    ...
    def cache_flights(self, flight_date):
        ...
        # Stupid database...
        if len(flight_ids) > 1:
            flights = FlightEntity.select().join(PlanetEntity)
            for flight in flights:
                self.flight_cache[flight.id] = flight
        elif flight_ids and flight_ids[0] not in self.flight_cache:
            flight = FlightEntity.select().join(PlanetEntity).where(FlightEntity.id == flight_ids[0]).get()
            self.flight_cache[flight.id] = flight
        return flight_ids
